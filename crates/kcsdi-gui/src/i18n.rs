...
catalog! {
    ...
    SourceWheelHelp => ("Scroll a digit to adjust its place. Click to type.", "在数字位上滚动调整，点击可输入。"),
    ...
    CalibrationContinuousOnly => ("User calibration requires Range mode.", "用户校准需要切换到范围模式。"),
    ...
    SegmentsTab => ("Segments", "分段"),
    Segment => ("Segment", "分段"),
    EditSegments => ("Edit segments", "编辑分段"),
    AddSegment => ("Add next segment", "添加下一段"),
    SegmentCount => ("Segments", "段数"),
    SegmentMaxStep => ("Maximum step", "最大步进"),
    SegmentCalculatedStep => ("Calculated step", "计算步进"),
    SegmentAcquiredPoints => ("Acquired points", "采集点数"),
    SegmentCompletedPoints => ("Last completed points", "上次完成点数"),
    SegmentsHelp => ("Adjacent segments share an endpoint. Set a maximum step for each segment.", "相邻分段共用端点，每段可设置不同的最大步进。"),
    SegmentStepHelp => ("Point count is calculated from the maximum nominal step. Reported frequencies may differ from this grid.", "按最大名义步进计算点数，设备返回频率可能与计算网格不同。"),
    SegmentWholeHz => ("Enter whole Hz, for example 5 kHz or 0.05 MHz.", "请输入整数赫兹，例如 5 kHz 或 0.05 MHz。"),
    SegmentOrderError => ("Stop must exceed start.", "终止频率必须大于起始频率。"),
    SegmentStepError => ("Maximum step must be positive.", "最大步进必须大于零。"),
    SegmentAdjacentError => ("Start must equal the previous segment's stop.", "起始频率必须等于上一段的终止频率。"),
    SegmentRequiredPoints => ("Required points", "所需点数"),
    SegmentMaximumPoints => ("Maximum points", "点数上限"),
    SegmentPointsHelp => ("Increase the step or split this segment.", "请增大步进或拆成多段。"),
    SegmentBudgetError => ("The plan exceeds the acquired-point limit.", "分段计划超过采集点数上限。"),
    SegmentReceiverError => ("Check this trace's calibration, bandwidth and format.", "请检查此轨迹的校准、带宽和格式。"),
    SegmentFrequencyLimits => ("Frequency limits", "频率范围"),
    SegmentMinimumSpan => ("Minimum span", "最小频宽"),
    SegmentSelectTrace => ("Select at least one visible trace.", "请至少显示一条轨迹。"),
    SegmentStopToApply => ("Stop acquisition before applying this plan.", "停止采集后才能应用分段计划。"),
    SegmentBoundsHelp => ("Apply keeps display bounds. Use Fit to show the full sweep.", "应用时保留显示范围，使用适应范围查看完整扫描。"),
    SegmentCalWarning => ("User calibration coverage for this segmented grid is unverified.", "用户校准对当前分段频率的适用性尚未验证。"),
    RangeTo => ("to", "至"),
