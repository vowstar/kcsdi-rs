...
![同一次 S11 扫描回放的中文阻抗和史密斯图界面](https://github.com/user-attachments/assets/e947a9de-02d2-4304-bc1e-ebcc84a5de96)
...
| S21 | 相位、损耗和群延迟 |
| 频谱 | 实时曲线，支持线性和对数频率轴 |
| 信号源 | KC901V RF 和 AF 频率、幅度与调制控制 |
| 校准 | KC901V S11 和 S21 系统校准与用户校准引导 |
| 数据 | CSV、XLSX 和 Touchstone `.s1p`。CLI 可将四组复数测量合成 `.s2p` |
| 分析 | 曲线显隐、保持、最大值与最小值包络、标记、缩放、平移和自动适配 |
| 界面 | 中文和英文，保存设备，支持浅色、深色和跟随系统主题 |

S11 和频谱实机测试使用 KC901V，固件版本为 V1.6.1。S21、信号源、校准、串口和局域网发现已做软件和本地回放测试。其他 KC901 型号仍需实机测试。
...
选择列表后点击编辑列表，设置各个频点。运行旁的设置可开启 CSV 或 XLSX 自动保存，并设置每轮后的等待时间。

信号源参数修改后，点击启动输出或应用才会发送。校准界面显示仪器提示，等待确认后继续。
